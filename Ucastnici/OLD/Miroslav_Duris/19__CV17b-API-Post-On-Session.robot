...
# Registrace na kurz využívající klíč. slovo 'Post On Session'. (tedy posílání více zpráv do jedné session)
    # 1. vytoření JSON zprávy (neboli 'body' requestu)
    # 2. vytoření hlavičky ('header') zprávy (zprávou myšlen 'request')
    # 3. vytvoření spojení ('sessionv')
    # 4. odeslání zprávy ('requestuv') a uložení odpovědi do proměnné ${resp}
    # 5. vyhodnocení
...
#
#   # vytoření hlavičky ('headerv') zprávy
#   &{header}=   Create Dictionary   Content-Type=application/json
#
#   # vytvoření spojení ('sessionv')
#   Create Session       apilogin            ${url}
#
